...
## 暗色主题 deck 专项规范（2026-06 实测）

暗色背景（`#0a0908` 级）deck 有几个坑是浅色主题不会碰到的，实测记录如下。

### 封面高亮框可读性

在暗底上叠加彩色信息框（如红色/橙色矩形）时，**框内文字必须同时满足**：

1. **对比度 ≥ 4.5:1**（WCAG AA）—— 暗背景上用白字没问题，但框本身不能和背景太接近
2. **字重 ≥ 600**（甚至是 700）—— 小字号在深色框上显得更淡，必须用粗体补偿
3. **字号 ≥ 14px（0.875rem）**—— 小于这个在移动端 / 小屏上基本无法阅读
4. **版本号 / 关键词单独上浅亮色**（如 `#ffe8d6`），和普通文字拉开层次

**反例**：直接用 `#c8895a`（铜橙）作为红框背景 + 白字，框和背景整体明度接近，文字在框内对比度暴跌。

### 绝对定位封面的可靠写法

多文件架构下，封面经常需要通过 `.cover-inner` 等容器居中。实测最可靠的方式：
...
flexbox `justify-content: center; align-items: center;` 在嵌套场景下（父级有 display:flex 或其他布局上下文）经常失效。**绝对定位 + translate 是唯一不依赖父级上下文的方法**。

### 正文字号设定策略

暗底 + 低对比文字组合下，正文 **最小 14px（0.875rem）**，行高 **≥ 1.75**。小字号 + 灰色文字在暗底上会「融入」背景，阅读成本指数上升。

#### ⚠️ fit 缩放的字号补偿

`deck_index.html` 演示模式会把整页 1920×1080 等比缩放（fit 到浏览器窗口）。用户感知字号 = CSS px × 缩放比。若笔记本屏幕可视区域 ≈ 1300×700，缩放比约 **0.68×**——源文件里设的 20px 会显得像 13.6px。

**补偿公式**：若投影 / 屏幕缩放比 ≤ 0.75，正文从 16px 直接提到 **20–24px**（1.25–1.5rem）。实测：
- 正文 `1.25rem`（20px）-> 缩后约 13.6px，仍偏小
- 正文 `1.5rem`（24px）-> 缩后约 16.4px，刚好可读

推荐基准（暗底 deck）：
- 正文：**20–24px**（1.25–1.5rem），行高 1.75
- 小节副标题：**20–25px**（1.25–1.6rem）
- 内容页大标题：`clamp(2.2rem, 4.5vw, 3.6rem)`（约 35–58px，不变）
- 封面主标题：`clamp(3.2rem, 7.5vw, 6.5rem)`（约 51–104px，不变）

#### ⚠️ 修改 tokens.css 后必须同步单页内联规则

多文件架构每页独立 `<style>`，若某页内联了 `.body-text { font-size: Xrem }`，**内联规则会覆盖 shared/tokens.css 的全局设置**。

**修改流程**：
1. 先改 `shared/tokens.css` 里对应的 class
2. 再 grep 所有 `slides/*.html` 的内联规则：`grep -n "font-size.*rem" slides/*.html`
3. 把同类选择器（`.body-text`、`.slide-subtitle`、`.mech-desc` 等）同步更新
4. 验证：`curl http://localhost/slides/02-background.html | grep font-size`

如果只改了 `tokens.css` 没改单页内联，改动会「看起来没生效」。

### 配色 token 命名规范

暗色 deck 的 CSS 变量建议分层：

```css
:root {
 --bg: #0a0908; /* 主背景 */
 --surface: #141210; /* 卡片/面板背景，比 bg 亮一点 */
 --border: #2a2420; /* 边框，微弱于文字 */
 --copper: #c8895a; /* 主 accent */
 --copper2: #e2a374; /* 亮 accent（hover、高亮） */
 --text: #faf8f5; /* 主文字 */
 --muted: #c4bfb9; /* 次要文字，正文 */
 --muted2: #8a8580; /* 最弱文字，页脚/标签 */
}
```

`--muted` 和 `--muted2` 两级灰色是必须的——只用一个灰色会导致「重要信息和页脚一样灰」。
