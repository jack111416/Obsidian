...
§
User prefers visual knowledge bases combining screenshots + transcripts. Expects B站视频提取工作流：元数据 → 音频下载 → ASR → markdown 块 → 摘要 → 可视化元素。
§
...
§
Termux China npm: use registry=https://registry.npmmirror.com. External agent skills install via npx to ~/.agents/skills/<name>/ (needs Chrome/Chromium/Edge for PTTX/PDF export — set CHROME_PATH).